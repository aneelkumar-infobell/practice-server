...
func HandleFunc() {
	...
	mux.HandleFunc("/add_asset_request", func(w http.ResponseWriter, r *http.Request) {
		...
		Asset_Id = Asset_Id + 1
		addStatement := `INSERT INTO asset (Asset_Name,Manufacturer, BMC_IP, BMC_User, BMC_Password, Asset_location,Reserved,Assigned_by,Assigned_to,Assigned_from,Created_on,Created_by,OS_IP,OS_User,OS_Password,Purpose,Cluster_Id,Delete) VALUES ($1,$2,$3,$4,$5,$6,'t',$7,$8,CURRENT_DATE,CURRENT_DATE,$9,$10,$11,$12,$13,$14,'0')`
		_, err = db.Exec(addStatement, assets.Asset_Name, assets.Manufacturer, assets.BMC_IP, assets.BMC_User, assets.BMC_Password, assets.Asset_Location, assets.Assigned_by, assets.Assigned_to, assets.Created_by, assets.OS_IP, assets.OS_User, assets.OS_Password, assets.Purpose, assets.Cluster_Id)
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
			fmt.Println(err)
			json.NewEncoder(w).Encode(map[string]interface{}{"Message": "Invalid input syntax for IP ", "Status Code": "400 ", "Error": err})
	...
	mux.HandleFunc("/Billing", func(w http.ResponseWriter, r *http.Request) {
		...
		var start time.Time
		var end time.Time
		err := db.QueryRow("SELECT Assigned_from,updated_on From Asset").Scan(&start, &end)
		if err != nil {
			log.Fatal(err)
		}
		fmt.Println(start)
